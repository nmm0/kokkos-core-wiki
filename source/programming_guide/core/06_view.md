...
* A Kokkos {cpp:class}`View` is an array of zero or more dimensions
...
[^exemption]: An exemption is if you specified for CUDA compilation that the default memory space is CudaUVMSpace, which can be accessed from the host.
